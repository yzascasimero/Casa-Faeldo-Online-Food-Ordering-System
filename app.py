...
from models import db, Admin, Customer, Product, Order, OrderItem, Reservation, Notification
...
# Jinja2 filter to convert UTC to Philippine Time (UTC+8)
...
def admin_dashboard():
    ...
    pending_reservations_list = Reservation.query.filter_by(status='pending').order_by(Reservation.reservation_date.asc(), Reservation.reservation_time.asc()).limit(5).all()
    
    # Get unread notifications count
    try:
        unread_notifications_count = Notification.query.filter_by(is_read=False).count()
    except Exception as e:
        # If Notification table doesn't exist yet, set count to 0
...
def add_product():
    if request.method == 'POST':
        ...
        product = Product(name=name, description=description, price=price,
                          category=category, available=available, image_url=image_url, variant=variant)
...
def admin_add_product():
    ...
    variant = request.form.get('variant')
    product = Product(name=name, description=description, price=price, category=category, available=True, image_url=image_url, variant=variant)
    db.session.add(product)
    db.session.commit()
    flash('Product added successfully.', 'success')
    return redirect(url_for('admin_menu_management'))
...
def admin_update_product():
    ...
    if 'image' in request.files and request.files['image']:
        ...
        if image_file.filename:
            ...
            save_path = os.path.join(app.config['UPLOAD_FOLDER'], filename)
            image_file.save(save_path)
            product.image_url = '/' + save_path.replace('\\', '/')
    db.session.commit()
    flash('Product updated successfully.', 'success')
    return redirect(url_for('admin_menu_management'))
...
def admin_notifications():
    ...
    try:
        ...
        # If Notification table doesn't exist yet, return empty
        return jsonify({
            'notifications': [],
            'unread_count': 0
        })
...
def place_order():
    ...
    delivery_fee = 75.00 if order_type == 'delivery' else 0
    total += delivery_fee
    ...
    order = Order(
        ...
        placed_outside_hours=outside_business_hours
    )
    ...
    try:
        notification = Notification(
            order_id=order.id,
            message=f'New order #{order.id} from {customer_name} - {order_type.title()} - ₱{total:.2f}'
        )
        db.session.add(notification)
    except Exception as e:
        # If Notification table doesn't exist yet, continue without notification
